...
describe('Mood Log Endpoints', () => {
  ...
  describe('POST /api/v1/mood', () => {
    it('creates a mood log', async () => {
      const res = await authRequest().post('/api/v1/mood').send(validMoodLog);
    ...
    it('rejects unauthenticated request with 401', async () => {
      const res = await request.post('/api/v1/mood').send(validMoodLog);
    ...
    it('rejects missing required fields with 400', async () => {
      const res = await authRequest().post('/api/v1/mood').send({ mood: 'happy' });
  ...
  describe('GET /api/v1/mood', () => {
    ...
    it("does not return another user's logs", async () => {
      ...
      await authRequest().post('/api/v1/mood').send(validMoodLog);
  ...
  describe('GET /api/v1/mood/:id', () => {
    it('returns a single mood log by id', async () => {
      const createRes = await authRequest().post('/api/v1/mood').send(validMoodLog);
  ...
  describe('PUT /api/v1/mood/:id', () => {
    ...
    it('returns 404 for non-existent id', async () => {
      const fakeId = '507f1f77bcf86cd799439011';
      const res = await authRequest().put(`/api/v1/mood/${fakeId}`).send({ mood: 'calm' });
    ...
    it('rejects invalid update data with 400', async () => {
      ...
      const res = await authRequest().put(`/api/v1/mood/${id}`).send({ intensity: 999 });
